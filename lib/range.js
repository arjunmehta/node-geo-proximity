...
function getHashArrayFromRadius(lat, lon, radius) {

    var range_depth = rangeDepth(radius),
        hash = geohash.encode_int(lat, lon, range_depth);

    return {
        bitDepth: range_depth,
        hashArray: buildBoxSet(hash, range_depth)
    };
}
...
function getQueryRangesFromBitDepth(lat, lon, radiusBitDepth, bitDepth, caching) {
    ...
    var hash = geohash.encode_int(lat, lon, radiusBitDepth),
        neighbors, ranges, cached;

    if (caching === true) {
        cached = checkCache(lat, lon, radiusBitDepth);
        if (cached !== false) {
            return cached;
        }
    }

    neighbors = buildBoxSet(hash, radiusBitDepth);
    neighbors.sort();
...
module.exports = exports = {
    ...
    getHashArrayFromRadius: getHashArrayFromRadius,
    bitDepthForRadius: rangeDepth
};
